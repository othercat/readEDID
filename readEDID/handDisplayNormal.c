...
int modeDisplayNormal(BLContextPtr context, struct clarg actargs[klast]) {
    //int ret = 0;
	...
	//if(actargs[kgetfulldata].present)
	{
        ...
        if (EDIDValue) {	/* this will fail on i.e. televisions connected to powerbook s-video output */
            ...
            if ((int)CFDataGetLength(EDIDValue)>128)
                blesscontextprintf(context, kBLLogLevelNormal,  "Get Ex block, please use <--getFullData> to get the detail\n");
