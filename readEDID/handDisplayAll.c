...
extern int parse_extb(BLContextPtr context,byte* extb);

int modeDisplayAll(BLContextPtr context, struct clarg actargs[klast]) {
    //int ret = 0;
	...
	{
        unsigned char i, EDID[128],EDID_b[128];
        CFRange allrange = {0, 128};
        CFRange range_ex = {128, 256};
        ...
        blesscontextprintf(context, kBLLogLevelNormal,  "EDID size: %d\n",(int)CFDataGetLength(EDIDValue));
        ...
        if (EDIDValue) {	/* this will fail on i.e. televisions connected to powerbook s-video output */
            ...
            for (i = 0; i < 128; i++){
                blesscontextprintf(context, kBLLogLevelNormal,  "%02X ", EDID[i] );
                if ((i+1)%16 == 0) blesscontextprintf(context, kBLLogLevelNormal,  "\n");
            }
            for (i = 0; i < 128; i++){
                if (isalpha(EDID[i]))
            ...
            parse_edid_full(context,EDID);
            
            if ((int)CFDataGetLength(EDIDValue)>128)
            {
                CFDataGetBytes(EDIDValue, range_ex, EDID_b);
                
                blesscontextprintf(context, kBLLogLevelNormal,  "Ex Block\n");
                for (i = 0; i < 128; i++){
                    blesscontextprintf(context, kBLLogLevelNormal,  "%02X ", EDID_b[i] );
                    if ((i+1)%16 == 0) blesscontextprintf(context, kBLLogLevelNormal,  "\n");
                }
                for (i = 0; i < 128; i++){
                    if (isalpha(EDID_b[i]))
                        blesscontextprintf(context, kBLLogLevelVerbose,  "%c ", EDID_b[i] );
                    else
                        blesscontextprintf(context, kBLLogLevelVerbose,  "  " );
                    if ((i+1)%16 == 0) blesscontextprintf(context, kBLLogLevelVerbose,  "\n");
                }
                blesscontextprintf(context, kBLLogLevelVerbose,  "\n");
                parse_extb(context,EDID_b);
            }
